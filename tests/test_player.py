...
def test_jump_input_from_idle_changes_state_to_jumping(player, world_state):
    ...
    assert isinstance(player.state, IdleState)
    ...
    # Assert (Então)
    assert isinstance(player.state, JumpingState)
...
def test_jumping_player_transitions_to_falling_at_jump_apex(player):
    ...
    player.state.update(player, {})  # Chamamos o update do estado

    # Assert (Então)
    assert isinstance(player.state, FallingState)
...
def test_wall_jump_action_and_transition(player):
    ...
    assert isinstance(player.state, WallSlidingState)
...
def test_can_dash_from_idle_state(player, world_state):
    ...
    player.change_state(IdleState())  # troca o estado para idle
    assert isinstance(player.state, IdleState)  # checa se está em idle
    ...
    assert isinstance(player.state, DashState)
    assert player.x_vel != 0
...
def test_dashing_state_reverts_to_idle_after_duration(player):
    # 1. Arrange
